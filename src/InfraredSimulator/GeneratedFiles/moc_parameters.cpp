/****************************************************************************
** Meta object code from reading C++ file 'parameters.h'
**
** Created by: The Qt Meta Object Compiler version 63 (Qt 4.8.5)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "Stdafx.h"
#include "parameters.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'parameters.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.5. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_parameters[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       8,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
      18,   12,   11,   11, 0x0a,
      46,   11,   11,   11, 0x0a,
      57,   11,   11,   11, 0x0a,
      70,   11,   11,   11, 0x0a,
      84,   11,   11,   11, 0x0a,
      95,   12,   11,   11, 0x0a,
     118,   11,   11,   11, 0x0a,
     134,   11,   11,   11, 0x0a,

       0        // eod
};

static const char qt_meta_stringdata_parameters[] = {
    "parameters\0\0index\0onSelectPreset(QModelIndex)\0"
    "onBrowse()\0onBeginSim()\0onAddNewRow()\0"
    "onDelRow()\0onComboBoxChanged(int)\0"
    "onEditChanged()\0onSaveFile(bool)\0"
};

void parameters::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        parameters *_t = static_cast<parameters *>(_o);
        switch (_id) {
        case 0: _t->onSelectPreset((*reinterpret_cast< const QModelIndex(*)>(_a[1]))); break;
        case 1: _t->onBrowse(); break;
        case 2: _t->onBeginSim(); break;
        case 3: _t->onAddNewRow(); break;
        case 4: _t->onDelRow(); break;
        case 5: _t->onComboBoxChanged((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 6: _t->onEditChanged(); break;
        case 7: _t->onSaveFile((*reinterpret_cast< bool(*)>(_a[1]))); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData parameters::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject parameters::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_parameters,
      qt_meta_data_parameters, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &parameters::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *parameters::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *parameters::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_parameters))
        return static_cast<void*>(const_cast< parameters*>(this));
    return QDialog::qt_metacast(_clname);
}

int parameters::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 8)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 8;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
